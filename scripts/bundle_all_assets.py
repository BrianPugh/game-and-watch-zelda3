...
assets_in_intflash = [
    index for (index, location) in assets_location if location == "intflash"
]
assets_in_ram = [index for (index, location) in assets_location if location == "ram"]
assets_in_extflash = [
    index
    for index in range(165)
    if index not in assets_in_intflash and index not in assets_in_ram
]
        ...
        with open(path, "wb") as output:
            ...
            for asset in range(nbAssets):
                ...
                if asset in assets_to_keep:
                    ...
                    print(
                        f"Asset #{asset}: {length} bytes @ 0x{addr:05x} --> @ Ox{addr_out:05x}"
                    )
            ...
            for asset in range(len(assets_to_keep)):
                ...
                addr_out = (addr_out + 3) & ~3
                print(
                    f"COPYING asset #{assets_to_keep[asset]}: {assets_data_length[asset]} bytes @ 0x{assets_data_addr[asset]:05x} --> @ Ox{addr_out:05x}"
                )
